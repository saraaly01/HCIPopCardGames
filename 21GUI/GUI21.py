...
def standAction():
    ...
    output.put("The dealer's hidden card was a" + str(dealer_hand[1]) + "Their score is now" + str(hand_score(dealer_hand)))
...
def insertImage(cardPlayed):
    ...
    global root
    width = int(250/2.5) #the width and height of the cards are resized because the actual jpeg file is too big for the screen
    height = int(363/2.5)    
...
def main(rootIN):
    ...
    dealerScore.place(relx=.8, rely = .1) #adds the dealers score on the GUI and then deals their second card so that the user doesn't know it
    ...
    #might be removed
    playAgainButton = Button(root, text="Play Again", font=("Comic Sans MS", 15),command=lambda: playAgain(rootIN), state=DISABLED)
